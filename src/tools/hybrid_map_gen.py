...
class HybridMapGenerator:
            ...
            for r_idx in range(rows):
                ...
                if shape_type == "circle":
                    ...
                    if shape_type == "square":
                        ...
                        orientation = get_sampled_value(cfg["triangle_orientation"])
                        add_triangle(
                            self.grid, final_cx, final_cy, size, angle, orientation
                        )
    ...
    def _generate_step_urban_section(self):
        ...
        if not cfg.get("enable_step", True):
            return 0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate hybrid maps using a master config."
    )
    parser.add_argument(
        "-c",
        "--config",
        default="master_config_2.yaml",
